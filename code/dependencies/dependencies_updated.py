...
for token in doc:
    print(token.head.text, token.dep_, token.text, token.head.pos_,
...
################################# TEXTUAL CONNECTIVES ######################################
    ...
    for verb in sent:
            if verb.pos == VERB:
                head_start = verb.idx
                head_end = verb.idx + len(verb.text) - 1
                for m in verb.children:
                    if m.dep == mark and str(m) == "che":
                        child_start = m.idx
                        child_end = m.idx + len(m.text) - 1
    ...
    for int in sent:
        if int.dep_ in ("punct") and str(int) == "?":
            ...
            head_end = int.head.idx + len(int.head.text) - 1
            child_start = int.idx
            child_end = int.idx + len(int.text) - 1
    ...
    for conj in sent:
        if conj.dep_ == cc and conj.text == "ma" or conj.text == "Ma":
            ...
            head_end = conj.head.idx + len(conj.head.text) - 1
            child_start = conj.idx
            child_end = conj.idx + len(conj.text) - 1
    ...
    for word in sent:
        if word.text == "per" and word.dep == mark:
            ...
            head_end = word.head.idx + len(word.head.text) - 1
            child_start = word.idx
            child_end = word.idx + len(word.text) - 1
...
############################## MORPHOLOGICAL ELEMENTS ############################################
        ...
        if verb.pos == VERB and verb in verb_list:
            for poss_subj in verb.children:
                if poss_subj.dep == nsubj:
                    ...
                    head_end = (poss_subj.head.idx + len(poss_subj.head.text) - 1)
                    child_start = poss_subj.idx
                    child_end = (poss_subj.idx + len(poss_subj.text) - 1)
    ...
    for pron in sent:
        if pron in pron_list:
            ...
            head_end = (pron.head.idx + len(pron.head.text) - 1)
            child_start = pron.idx
            child_end = (pron.idx + len(pron.text) - 1)
